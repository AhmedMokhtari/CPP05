...
void Bureaucrat::executeForm(AForm const & form) const {
    if (!form.getIsSign())
    {
        std::cout << "Form not signed for execute it " << std::endl;
        return ;
    }
    if (form.getGrade_ex() < this->grade)
    {
        std::cout << "Bureaucrat don't have enough grade to execute it" << std::endl;
        return;
    }
